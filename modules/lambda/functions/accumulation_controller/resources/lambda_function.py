...
def setup_logger(name):
    ...
    # Lambda ロガーでは ルートロガー がデフォルト出力をするため伝播を抑制する
...
def keep_gold_status():
    ...
    for i in range(0, required_units, each_units * 2):
        long_order_data = oanda.trade._make_place_order_data(
            units=each_units, instrument="USD_JPY"
        )
        # 発注
        place_order_response = oanda.trade.request_place_order(long_order_data)
        logger.info(f"USD_JPY を {each_units} 枚発注しました")
        # 決済
        trade_id = place_order_response["orderFillTransaction"]["id"]
        close_order_data = oanda.trade._make_close_order_data(units="ALL")
        oanda.trade.request_close_order(trade_id=trade_id, close_data=close_order_data)
        logger.info(f"USD_JPY を {each_units} 枚決済しました")
